  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link to="/production/new" className="btn-primary flex items-center gap-2">
            <Plus size={18} />
            <span className="hidden sm:inline">Lancer une Production</span>
          </Link>
          <button
            onClick={fetchProductions}
            className="p-2.5 text-muted-500 hover:text-primary-500 bg-surface border border-muted-300 rounded-btn hover:border-primary-300 transition-all"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-surface p-4 sm:p-5 rounded-card border border-muted-300 shadow-card col-span-2 sm:col-span-1">
      ...
      {/* Table — desktop only */}
      <div className="hidden sm:block bg-surface rounded-card shadow-card border border-muted-300 overflow-hidden">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-navy">
                          {prod.actual_quantity || prod.planned_quantity}
                          <span className="text-[10px] text-muted-400 font-normal ml-1">{prod.product?.unit}</span>
                        </span>
                      ...
                      <div className="text-[11px] text-muted-500 space-y-0.5">
                        ...
                        {prod.completed_at && (
                          <div className="flex items-center gap-1 text-success font-medium">
                            <CheckCircle2 size={10} /> Fin: {fmtDate(prod.completed_at)}
                          </div>
                        )}
                      </div>
                    </td>
        ...
        {loading ? (
          [...Array(3)].map((_, i) => (
            <div key={i} className="p-4 animate-pulse">
              <div className="h-4 bg-muted-100 rounded w-1/3 mb-2" />
          ...
          <div className="p-8 text-center text-muted-500">
            ...
            <p className="text-sm">Aucun ordre de fabrication en cours.</p>
          </div>
        ) : productions.map((prod) => {
          const st = STATUS_MAP[prod.status] || STATUS_MAP.pending
          return (
            <div key={prod.id} className="p-4 hover:bg-muted-50/50 transition-colors">
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="min-w-0 flex-1">
                  <div className="font-black text-primary-600 text-[10px] tracking-wider mb-0.5">{prod.reference}</div>
                  <div className="font-bold text-navy truncate">{prod.product?.name}</div>
                  <div className="text-[10px] text-muted-500 flex items-center gap-1 mt-0.5">
                    <Tag size={10} /> {prod.bom?.name || 'Standard'}
                  </div>
                </div>
                <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shrink-0", st.color)}>
                  <st.icon size={10} /> {st.label}
                </span>
              </div>
              <div className="flex items-center justify-between mt-3">
                <div className="text-xs text-muted-500 flex items-center gap-1">
                  <Clock size={10} /> {fmtDate(prod.started_at || prod.created_at)}
                </div>
                <Link
                  to={`/production/${prod.id}`}
