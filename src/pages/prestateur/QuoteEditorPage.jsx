...
// ── Ligne de devis — desktop (table row) ─────────────────────────────────────
...
// ── Ligne de devis — mobile (card) ────────────────────────────────────────────
function ItemCard({ item, index, onChange, onRemove, isLast }) {
  const update = (field, value) => {
    const updated = { ...item, [field]: value }
    updated.total = (Number(updated.quantity) || 0) * (Number(updated.unit_price) || 0)
    onChange(index, updated)
    ...
    <div className="space-y-4 sm:space-y-5 print:space-y-3">
      ...
      <div className="flex flex-col gap-3 print:hidden">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link to="/prestateur/quotes" className="btn-outline text-sm hidden sm:inline-flex">
            Annuler
          </Link>
          <button
            onClick={handleSave}
      ...
      {/* ── Formulaire ── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 print:grid-cols-2 print:gap-3">
